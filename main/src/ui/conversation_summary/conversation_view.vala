...
public class ConversationView : Box, Plugins.ConversationItemCollection, Plugins.NotificationCollection {
    ...
    MessageItem current_highlighted_item = null;
    ...
    public ConversationView init(StreamInteractor stream_interactor) {
        ...
        main_wrap_event_box.leave_notify_event.connect((event) => {
            print("exit\n");
            mouse_inside = false;
        ...
        main_wrap_event_box.enter_notify_event.connect((event) => {
            print("enter\n");
            mouse_inside = true;
        ...
        main_event_box.motion_notify_event.connect((event) => {
            mouse_inside = true;
            update_highlight((int)event.x_root, (int)event.y_root);
            return false;
        });
        ...
        chooser.emoji_picked.connect((emoji) => {
            if (current_highlighted_item.message == null) return;
            stream_interactor.get_module(Reactions.IDENTITY).add_reaction(conversation.account, current_highlighted_item.message, emoji);
        });
    ...
    private void update_highlight(int x_root, int y_root) {
        ...
        while (iter.next()) {
            if (iter.get_value() == w) {
                ...
                var message_item = meta_content_item.content_item as MessageItem;
                if (message_item == null) return;
                current_highlighted_item = message_item;
            }
        }
